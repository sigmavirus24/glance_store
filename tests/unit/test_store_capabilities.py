...
class TestStoreCapabilityConstants(object):

    def test_one_single_capability_own_one_bit(self):
        cap_list = [
            caps.BitMasks.READ_ACCESS,
            caps.BitMasks.WRITE_ACCESS,
            caps.BitMasks.READ_OFFSET,
            caps.BitMasks.WRITE_OFFSET,
            caps.BitMasks.READ_CHUNK,
            caps.BitMasks.WRITE_CHUNK,
            caps.BitMasks.DRIVER_REUSABLE,
        ]
        for cap in cap_list:
    ...
    def test_combined_capability_bits(self):
        # Test the RW_* capabilities
        caps.RW_ACCESS = (caps.BitMasks.READ_ACCESS |
                          caps.BitMasks.WRITE_ACCESS)
        caps.RW_OFFSET = (caps.BitMasks.READ_OFFSET |
                          caps.BitMasks.WRITE_OFFSET)
        caps.RW_CHUNK = (caps.BitMasks.READ_CHUNK | caps.BitMasks.WRITE_CHUNK)
        caps.RW_RANDOM = (caps.BitMasks.READ_RANDOM |
                          caps.BitMasks.WRITE_RANDOM)

        # Test the cascaded read and write capabilities
        check = caps.StoreCapability._x_contains_y
